package day02methodcreation;

public class Car {
    public String marka = "Honda";
    public int fiyat = 20000;

    public static void main(String[] args) {
    // object nasıl olusturulur?
    // 1)Class ismini yazınız
    // 2)Object e isim verin
    // 3)= operatörü koyarız
    // 4)new key wordunu kullan
    // 5)class ismi parantezle beraber yazıyoruz.
    // En basa obje olusturacagımız classın(kalıp) ismini yazıyoruz
    Car myHonda = new Car();
        System.out.println(myHonda.fiyat);//20000
        System.out.println(myHonda.marka);//Honda
        //Alttakileri yazdırın içine koymaya gerek yok cünkü o method zaten yazdırma işi yapıyor.
        myHonda.hareketEt();
        myHonda.dur();
    MethodCreation obj= new MethodCreation();
        System.out.println(obj.toplamaYap(3,5));
    }
    public void hareketEt(){
        System.out.println("Honda guzel hareket eder...");
    }
    public void dur(){
        System.out.println("Honda guvenli durur");
    }


}
